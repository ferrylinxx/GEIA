    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-zinc-800">Categorías de Memoria</h2>
        </div>
        <button
      ...
      {/* Create form */}
      {showCreate && (
        <form onSubmit={createCategory} className="bg-white/80 backdrop-blur-sm border border-zinc-200 rounded-xl p-6 space-y-4 shadow-lg">
                ...
                {COLOR_OPTIONS.map(color => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full transition-all border-2 ${
                      newCategory.color === color ? 'ring-2 ring-blue-500 scale-110 border-white' : 'border-zinc-300'
                    }`}
          ...
          <button
            type="submit"
            className="w-full px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors shadow-lg"
          >
            Crear Categoría
          </button>
        </form>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {categories.map(category => (
          <div
            key={category.id}
            className="bg-white/80 backdrop-blur-sm border border-zinc-200 rounded-xl p-4 hover:border-blue-400 hover:shadow-xl transition-all"
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-zinc-800">{category.name}</h3>
                  <p className="text-sm text-zinc-600 font-medium">{category.memory_count || 0} memorias</p>
                </div>
              </div>
            </div>
            {category.description && (
              <p className="text-sm text-zinc-600 line-clamp-2">{category.description}</p>
            )}
          </div>
        ))}
      </div>
